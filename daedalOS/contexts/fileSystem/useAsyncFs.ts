...
const statTimeMs = (
  stats: Stats,
  msKey: "atimeMs" | "mtimeMs" | "ctimeMs" | "birthtimeMs",
  dateKey: "atime" | "mtime" | "ctime" | "birthtime"
): number => {
  const raw = stats as unknown as Record<string, unknown>;
  const maybeMs = raw[msKey];

  if (typeof maybeMs === "number") return maybeMs;
            ...
            return resolve(
              stats.size === -1 && isExistingFile(stats)
                ? new Stats(
                    ...
                    get9pSize(path),
                    stats.mode,
                    new Date(statTimeMs(stats, "atimeMs", "atime")),
                    new Date(statTimeMs(stats, "mtimeMs", "mtime")),
                    new Date(statTimeMs(stats, "ctimeMs", "ctime"))
                  )
                : stats
            );
          });

...
# ────────────────────────────────────────────────
# DTO Working Ledger (off-book operational domain)
# ────────────────────────────────────────────────
class DtoProfile(db.Model):
    __tablename__ = 'dto_profile'
    __table_args__ = (
        db.UniqueConstraint('employee_id', name='uq_dto_profile_employee'),
    )
    ...
    employee = db.relationship('Employee', backref='dto_profiles', lazy='select')
    district = db.relationship('District', backref='dto_profiles', lazy='select')
    project = db.relationship('Project', backref='dto_profiles', lazy='select')
    created_by = db.relationship('User', backref='created_dto_profiles', lazy='select')

    @property
    def dto_name(self):
        return self.employee.name if self.employee else f'DTO#{self.id}'

    def __repr__(self):
        return f'<DtoProfile {self.id} emp={self.employee_id}>'


class DtoCounterparty(db.Model):
    __tablename__ = 'dto_counterparty'
    __table_args__ = (
        db.UniqueConstraint('dto_profile_id', 'counterparty_type', 'name', name='uq_dto_counterparty_scope'),
    )

    id = db.Column(db.Integer, primary_key=True)
    dto_profile_id = db.Column(db.Integer, db.ForeignKey('dto_profile.id', ondelete='CASCADE'), nullable=False, index=True)
    counterparty_type = db.Column(db.String(20), nullable=False, index=True)  # Driver / Party / Market / Other
    name = db.Column(db.String(150), nullable=False, index=True)
    driver_id = db.Column(db.Integer, db.ForeignKey('driver.id', ondelete='SET NULL'), nullable=True)
    party_id = db.Column(db.Integer, db.ForeignKey('party.id', ondelete='SET NULL'), nullable=True)
    phone = db.Column(db.String(30), nullable=True)
    account_ref = db.Column(db.String(80), nullable=True)
    is_active = db.Column(db.Boolean, default=True, nullable=False)
    created_at = db.Column(db.DateTime, default=pk_now)
    updated_at = db.Column(db.DateTime, default=pk_now, onupdate=pk_now)

    dto_profile = db.relationship('DtoProfile', backref='counterparties', lazy='select')
    driver = db.relationship('Driver', backref='dto_counterparty_links', lazy='select')
    party = db.relationship('Party', backref='dto_counterparty_links', lazy='select')

    def __repr__(self):
        return f'<DtoCounterparty {self.id} {self.counterparty_type}:{self.name}>'


class DtoSettlement(db.Model):
    __tablename__ = 'dto_settlement'

    id = db.Column(db.Integer, primary_key=True)
    settlement_number = db.Column(db.String(40), nullable=False, unique=True, index=True)
    ...
    posting_mode = db.Column(db.String(20), nullable=False, default='Journal')
    description = db.Column(db.Text, nullable=True)
    journal_entry_id = db.Column(db.Integer, db.ForeignKey('journal_entry.id', ondelete='SET NULL'), nullable=True, index=True)
    created_by_user_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='SET NULL'), nullable=True)
    posted_by_user_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='SET NULL'), nullable=True)
    created_at = db.Column(db.DateTime, default=pk_now)
    posted_at = db.Column(db.DateTime, nullable=True)
    updated_at = db.Column(db.DateTime, default=pk_now, onupdate=pk_now)

    dto_profile = db.relationship('DtoProfile', backref='settlements', lazy='select')
    district = db.relationship('District', backref='dto_settlements', lazy='select')
    project = db.relationship('Project', backref='dto_settlements', lazy='select')
    journal_entry = db.relationship('JournalEntry', backref='dto_settlements', lazy='select')
    created_by = db.relationship('User', foreign_keys=[created_by_user_id], backref='created_dto_settlements', lazy='select')
    posted_by = db.relationship('User', foreign_keys=[posted_by_user_id], backref='posted_dto_settlements', lazy='select')

    def __repr__(self):
        return f'<DtoSettlement {self.settlement_number} {self.status}>'


class DtoTxn(db.Model):
    __tablename__ = 'dto_txn'

    id = db.Column(db.Integer, primary_key=True)
    txn_number = db.Column(db.String(40), nullable=False, unique=True, index=True)
    txn_date = db.Column(db.Date, nullable=False, index=True)
    district_id = db.Column(db.Integer, db.ForeignKey('district.id'), nullable=False, index=True)
    project_id = db.Column(db.Integer, db.ForeignKey('project.id'), nullable=True, index=True)
    dto_profile_id = db.Column(db.Integer, db.ForeignKey('dto_profile.id', ondelete='CASCADE'), nullable=False, index=True)
    counterparty_id = db.Column(db.Integer, db.ForeignKey('dto_counterparty.id', ondelete='SET NULL'), nullable=True, index=True)
    counterparty_type = db.Column(db.String(20), nullable=False, index=True)
    counterparty_name = db.Column(db.String(150), nullable=False, index=True)
    mode = db.Column(db.String(20), nullable=False)  # Cash / Credit / Bank / Wallet
    txn_nature = db.Column(db.String(30), nullable=False)  # Purchase / Borrow / Lending / Payment / Receive / Adjustment
    direction = db.Column(db.String(20), nullable=False, index=True)  # Payable / Receivable
    amount = db.Column(db.Numeric(15, 2), nullable=False)
    due_date = db.Column(db.Date, nullable=True, index=True)
    is_company_claimable = db.Column(db.Boolean, default=False, nullable=False, index=True)
    claim_status = db.Column(db.String(20), nullable=False, default='Unclaimed', index=True)  # Unclaimed / Claimed / Settled
    status = db.Column(db.String(20), nullable=False, default='Draft', index=True)  # Draft / Verified / Settled / Cancelled
    reference_no = db.Column(db.String(80), nullable=True)
    notes = db.Column(db.Text, nullable=True)
    attachment_path = db.Column(db.String(500), nullable=True)
    settlement_id = db.Column(db.Integer, db.ForeignKey('dto_settlement.id', ondelete='SET NULL'), nullable=True, index=True)
    created_by_user_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='SET NULL'), nullable=True)
    verified_by_user_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='SET NULL'), nullable=True)
    settled_by_user_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='SET NULL'), nullable=True)
    created_at = db.Column(db.DateTime, default=pk_now)
    verified_at = db.Column(db.DateTime, nullable=True)
    settled_at = db.Column(db.DateTime, nullable=True)
    updated_at = db.Column(db.DateTime, default=pk_now, onupdate=pk_now)

    district = db.relationship('District', backref='dto_transactions', lazy='select')
    project = db.relationship('Project', backref='dto_transactions', lazy='select')
    dto_profile = db.relationship('DtoProfile', backref='transactions', lazy='select')
    counterparty = db.relationship('DtoCounterparty', backref='transactions', lazy='select')
    settlement = db.relationship('DtoSettlement', backref='transactions', lazy='select')
    created_by = db.relationship('User', foreign_keys=[created_by_user_id], backref='created_dto_transactions', lazy='select')
    verified_by = db.relationship('User', foreign_keys=[verified_by_user_id], backref='verified_dto_transactions', lazy='select')
    settled_by = db.relationship('User', foreign_keys=[settled_by_user_id], backref='settled_dto_transactions', lazy='select')

    def __repr__(self):
        return f'<DtoTxn {self.txn_number} {self.status}>'


class DtoSettlementLine(db.Model):
    __tablename__ = 'dto_settlement_line'
    __table_args__ = (
        db.UniqueConstraint('settlement_id', 'dto_txn_id', name='uq_dto_settlement_line_unique'),
    )

    id = db.Column(db.Integer, primary_key=True)
    settlement_id = db.Column(db.Integer, db.ForeignKey('dto_settlement.id', ondelete='CASCADE'), nullable=False, index=True)
    dto_txn_id = db.Column(db.Integer, db.ForeignKey('dto_txn.id', ondelete='RESTRICT'), nullable=False, index=True)
    amount = db.Column(db.Numeric(15, 2), nullable=False)
    direction = db.Column(db.String(20), nullable=False)
    note = db.Column(db.String(255), nullable=True)
    created_at = db.Column(db.DateTime, default=pk_now)

    settlement = db.relationship('DtoSettlement', backref='lines', lazy='select')
    dto_txn = db.relationship('DtoTxn', backref='settlement_lines', lazy='select')

    def __repr__(self):
        return f'<DtoSettlementLine set={self.settlement_id} txn={self.dto_txn_id}>'


class DtoAttachment(db.Model):
    __tablename__ = 'dto_attachment'

    id = db.Column(db.Integer, primary_key=True)
    dto_txn_id = db.Column(db.Integer, db.ForeignKey('dto_txn.id', ondelete='CASCADE'), nullable=True, index=True)
    dto_settlement_id = db.Column(db.Integer, db.ForeignKey('dto_settlement.id', ondelete='CASCADE'), nullable=True, index=True)
    file_path = db.Column(db.String(500), nullable=False)
    file_type = db.Column(db.String(20), nullable=True)
    original_name = db.Column(db.String(255), nullable=True)
    ...
    txn = db.relationship('DtoTxn', backref='attachments', lazy='select')
    settlement = db.relationship('DtoSettlement', backref='attachments', lazy='select')
    uploaded_by = db.relationship('User', backref='dto_attachments', lazy='select')

    def __repr__(self):
        return f'<DtoAttachment {self.id}>'

...
def workspace_party_form(pk=None):
    ...
    if request.method == "POST":
        ...
        if conf.get("exact"):
            ep = conf["exact"]
            flash(
                f'Yeh party pehle se maujood hai: ΓÇ£{ep.name}ΓÇ¥. (Same naam ΓÇö spacing/case alag ho sakta hai). Duplicate nahi bana sakte.',
                "danger",
            )
            return render_template(
                "workspace/party_form.html",
        ...
        if conf.get("similar") and request.form.get("ack_similar") != "1":
            ...
            flash(
                f'Yeh naam maujood party ΓÇ£{sp.name}ΓÇ¥ se bahut milta-julta hai (~{scr}% match). Agar wohi nayi party nahi, pehle wala edit karein; warna neeche tick karke save karein.',
                "warning",
            )
            return render_template(
                "workspace/party_form.html",
...
def workspace_product_form(pk=None):
    ...
    if request.method == "POST":
        ...
        if conf.get("exact"):
            ep = conf["exact"]
            flash(
                f'Yeh product pehle se maujood hai: ΓÇ£{ep.name}ΓÇ¥. (Same naam ΓÇö spacing/case alag ho sakta hai). Duplicate nahi bana sakte.',
                "danger",
            )
            return render_template(
                "workspace/product_form.html",
        ...
        if conf.get("similar") and request.form.get("ack_similar") != "1":
            ...
            scr = int(round(float(conf.get("similarity") or 0) * 100))
            flash(
                f'Yeh naam maujood product ΓÇ£{sp.name}ΓÇ¥ se bahut milta-julta hai (~{scr}% match). Agar wohi naya product nahi, pehle wala edit karein; warna neeche tick karke save karein.',
                "warning",
            )
            return render_template(
                "workspace/product_form.html",
...
def workspace_fund_transfer_view(pk):
    ...
    return render_template(
        ...
        rec=row,
        is_workspace=True,
        title="Workspace fund transfer ΓÇö " + (row.transfer_number or ""),
        back_url=back_url,
...
def workspace_fund_transfer_media(pk):
    ...
    sub = f"Transfer: {row.transfer_number} | {row.transfer_date.strftime('%d-%m-%Y') if row.transfer_date else 'ΓÇö'}"
    tmpl = dict(
        rec=row,
        media_items=media_items,
        media_title="Workspace fund transfer ΓÇö Media",
...
def workspace_transfer_slip_ocr_api():
    ...
    Order: OpenRouter free vision models (one by one) ΓåÆ Gemini Vision (if GOOGLE_API_KEY set).
    ...
    # Strip data URI prefix ΓÇö keep mime type
    mime_type = "image/png"
    ...
    # -----------------------------------------------------------------------
    # 1. OpenRouter ΓÇö try each free vision model in order
    # -----------------------------------------------------------------------
    ...
    if openrouter_key:
        ...
        # Free vision-capable models on OpenRouter ΓÇö ordered by OCR quality (best first)
        # Only models with image input modality are included
        _free_vision_models = [
            "google/gemini-2.0-flash-exp:free",            # #1 ΓÇö Gemini Flash, best OCR accuracy, fast
            "meta-llama/llama-4-maverick:free",            # #2 ΓÇö Llama 4 Maverick, large multimodal
            "meta-llama/llama-4-scout:free",               # #3 ΓÇö Llama 4 Scout, fast multimodal
            "qwen/qwen2.5-vl-72b-instruct:free",           # #4 ΓÇö Qwen VL 72B, strong vision
            "nvidia/nemotron-nano-12b-v2-vl:free",         # #5 ΓÇö Nvidia 12B vision
            "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free",  # #6 ΓÇö Nvidia Omni 30B
        ]
        ...
        for model_id in openrouter_models:
            try:
                ...
                if he.code in (429, 402, 503):
                    continue  # rate-limited or unavailable ΓÇö try next
                continue
            except Exception as exc:
